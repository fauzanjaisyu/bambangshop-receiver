...
impl NotificationService {
    pub fn subscribe(product_type: &str) -> Result<Subscriber> {
    ...
    async fn subscribe_request(product_type: String) -> Result<Subscriber> {
        ...
        return match request {
            Ok(f) => match f.json::<Subscriber>().await {
                Ok(x) => Ok(x),
    ...
    pub fn unsubscribe(product_type: &str) -> Result<Subscriber> {
        let product_type_clone = String::from(product_type);
    ...
    #[tokio::main]
    async fn unsubscribe_request(product_type: String) -> Result<Subscriber> {
        let product_type_upper: String = product_type.to_uppercase();
        let product_type_str: &str = product_type_upper.as_str();
        let notification_receiver_url: String = format!("{}/receive",
            APP_CONFIG.get_instance_root_url());
